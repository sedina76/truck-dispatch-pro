...
// Broker Profile -> Profitability. get_profitability_by_broker()
// (0037_profitability.sql) is already a GROUP BY over the canonical
// get_load_profitability() result -- filtered here to this one broker's
// row, not a separate calculation.
export async function BrokerProfitabilitySection({ brokerId }: { brokerId: string }) {
  const supabase = await createClient();
  const { data } = await supabase.rpc("get_profitability_by_broker", {}).eq("broker_id", brokerId).maybeSingle();
  const p = data as {
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
      ...
      <Link href={`/reports/profit-by-broker?dim=broker`} className="mt-2 inline-block text-xs font-medium text-[var(--color-brand)]">
