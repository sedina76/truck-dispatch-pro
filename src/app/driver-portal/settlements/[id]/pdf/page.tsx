...
// Driver Portal's own Settlement PDF (spec section 17) -- same browser-
// print layout as the staff /driver-settlements/[id]/pdf page, but gated
// by the driver-portal session + an explicit .eq("driver_id", ...)
// ownership check instead of staff RLS (drivers have no Supabase Auth
// session, same reasoning as every other driver-portal page). No new PDF
// generation/storage -- print-to-PDF via the browser, exactly like the
// staff version.
export default async function DriverPortalSettlementPdfPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: settlement } = await supabase
    ...
    .select(
      "id, settlement_number, period_start, period_end, status, gross_pay, adjustments_amount, deductions_amount, advances_amount, net_pay, amount_paid, balance_due, organization_id, driver_id, drivers(first_name, last_name), carriers(legal_name)"
    )
    .eq("id", id)
