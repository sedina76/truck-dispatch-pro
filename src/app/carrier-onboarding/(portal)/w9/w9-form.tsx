...
import { certifyAndGenerateMyW9, saveMyW9Draft, setMyW9Tin } from "../../actions";
import { TAX_CLASSIFICATION_LABELS, isValidTinFormat, type CarrierW9Row, type W9TaxClassification, type W9TinType } from "@/lib/carrier-w9/types";
...
export function W9Form({ w9 }: { w9: CarrierW9Row }) {
  ...
  const [tinType, setTinType] = useState<W9TinType>(w9.tin_type ?? "ein");
  ...
  function handleSaveDraft() {
    startSave(async () => {
      const result = await saveMyW9Draft(w9.id, {
  ...
  function handleCertify() {
    ...
    startCertify(async () => {
      // Save every field first so certify_carrier_w9()'s server-side
      // validation runs against exactly what's on screen, not a
      // possibly-stale prior save.
      const saveResult = await saveMyW9Draft(w9.id, {
      ...
      const tinResult = await setMyW9Tin(w9.id, tinType, tin);
      if (!tinResult.ok) { toast.show("error", tinResult.error); return; }
      const result = await certifyAndGenerateMyW9(w9.id, { certifiedName, certifiedTitle, tinType, tin });
      ...
      <section className="min-w-0 space-y-3">
        ...
        <Field label={tinType === "ssn" ? "Social Security Number" : "Employer Identification Number"}>
          <Input
            ...
            className={cn("w-full min-w-0 max-w-xs font-mono", tin && !isValidTinFormat(tin) && "border-destructive")}
          />
        </Field>
      ...
      <section className="min-w-0 space-y-3 border-t border-desktop-border pt-4">
        ...
        <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
          <Field label="Full Legal Name (electronic signature)">
            <Input value={certifiedName} onChange={(e) => setCertifiedName(e.target.value)} className={cn("w-full min-w-0 font-serif text-base italic", certifiedName && "border-primary")} />
          </Field>
          <Field label="Title (if signing on behalf of an entity)">
