// Mirrors src/lib/carrier-setup-packages/storage-retry.ts deliberately --
// same bounded-retry/confirm-after-response-failure strategy for an
// upload to private Storage. Duplicated rather than imported, matching
// this feature's established precedent (see types.ts's header comment):
// Broker Packet is a distinct business object, so its infrastructure can
// diverge later without silently changing Carrier Setup Package's behavior.
